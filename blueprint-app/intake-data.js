...
const INTAKE_SECTIONS = [
  ...
  { id: 's6', num: 6, title: 'Outbound & Load Requirements', scope: 'product',
    ...
    fields: [
      ...
      { id: 'slipsheetLayers',    label: 'Which layers?', type: 'select', showIf: { field: 'slipsheetsRequired', equals: 'Yes' }, options: ['Dust cover on top only','Top + layer 2','Top + between every layer','Between every layer only', INTAKE_CUSTOM_SENTINEL] },
      { id: 'slipsheetLayersCustom', label: 'Your slipsheet placement', type: 'text', helper: 'Custom values are flagged for Sojo review', showIf: { field: 'slipsheetLayers', equals: INTAKE_CUSTOM_SENTINEL } },
      ...
      { id: 'airbagReqs', label: 'Airbag requirements',   type: 'select', options: ['None','2 — front + rear of trailer','3 — beginning, middle, end of trailer','4+', INTAKE_CUSTOM_SENTINEL] },
      { id: 'airbagReqsCustom', label: 'Your airbag requirements', type: 'text', helper: 'Custom values are flagged for Sojo review', showIf: { field: 'airbagReqs', equals: INTAKE_CUSTOM_SENTINEL } },
  ...
  { id: 's7', num: 7, title: 'Sticker Requirements', scope: 'product',
    ...
    fields: [
      ...
      { id: 'stickerPlacement', label: 'Placement',           type: 'select', showIf: { field: 'stickersRequired', equals: 'Yes' }, options: ['Top of case','Front face','Side panel','Two adjacent sides','Over existing UPC', INTAKE_CUSTOM_SENTINEL] },
      { id: 'stickerPlacementCustom', label: 'Your sticker placement', type: 'text', helper: 'Custom values are flagged for Sojo review', showIf: { field: 'stickerPlacement', equals: INTAKE_CUSTOM_SENTINEL } },
      { id: 'stickersPerCase',  label: 'Number per case',     type: 'select', showIf: { field: 'stickersRequired', equals: 'Yes' }, options: ['1','2','3','4', INTAKE_CUSTOM_SENTINEL] },
      { id: 'stickersPerCaseCustom', label: 'Your stickers per case', type: 'number', helper: 'Custom values are flagged for Sojo review', showIf: { field: 'stickersPerCase', equals: INTAKE_CUSTOM_SENTINEL } },
  ...
  { id: 's9', num: 9, title: 'Artwork & Dielines', scope: 'product',
    ...
    fields: [
      ...
      { id: 'complianceArtwork',label: 'Compliance requirements on artwork', type: 'multiselect', helper: 'Check everything that must appear on the artwork', options: ['Allergen statement','Organic certification mark','Non-GMO','Kosher','Recycling / deposit marks (CRV, etc.)','Bilingual labeling','None','Other'] },
      { id: 'complianceArtworkNotes', label: 'Compliance details', type: 'textarea', helper: 'Specifics for anything checked above — exact statements, cert numbers, languages' },
    ]},
];
...
const BOM_ROW_FIELDS = [
  ...
  { id: 'count',        label: 'Count (CT)', type: 'select', options: ['1CT','4CT','6CT','12CT','15CT','24CT','Bulk', INTAKE_CUSTOM_SENTINEL] },
  { id: 'countCustom',  label: 'Your count (CT)', type: 'text', showIf: { field: 'count', equals: INTAKE_CUSTOM_SENTINEL } },
